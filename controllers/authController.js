...
async function signup(req, res, next) {
  try {
    ...
    // Auto-generate this new user's own referral code
    const refCode =
      phone.slice(-4) + crypto.randomBytes(4).toString('hex').toUpperCase();
    ...
    const expiresAt = new Date(
      Date.now() + jwtExpiryToMs(process.env.JWT_EXPIRES_IN)
    );

    await authService.saveSession(
      user.id, accessToken, expiresAt, deviceInfo, ipAddress
    );
...
async function sendOtp(req, res, next) {
  try {
    ...
    const devOtp =
      process.env.OTP_BYPASS_DEV === 'true' &&
  ...
  try {
    ...
    if (!user || !user.is_active) {
      return R.ok(
        res,
        null,
        `If an account exists for ${maskPhone(phone)}, a reset OTP has been sent.`
      );
    }
...
async function resetPassword(req, res, next) {
  try {
    ...
    if (!valid) return R.unauthorized(res, 'Invalid or expired OTP.');
    ...
    return R.ok(
      res, null, 'Password updated successfully. Please log in again.'
    );
  } catch (err) {
    next(err);
  }
...
async function refreshToken(req, res, next) {
  try {
    ...
    await authService.saveSession(
      user.id, newAccess, expiresAt, deviceInfo, ipAddress
    );
...
async function changePassword(req, res, next) {
  try {
    ...
    const userRow = await authService.findUserById(req.user.id);
    if (!userRow) return R.notFound(res, 'User not found.');

    const user  = await authService.findUserByPhone(userRow.phone);
