/**
 * services/planService.js  (with coupon support)
 *
 * purchasePlan now accepts an optional couponCode.
 * The referral-auto-discount from the old version is removed —
 * referral benefit is now delivered as a coupon (generated by couponService).
 */
...
async function purchasePlan(userId, planId, paymentMode = 'wallet', couponCode = null) {
  ...
  return db.transaction().execute(async (trx) => {
    ...
    const notifBody = discountAmount > 0
    ...
    await trx
      ...
      .values({
        user_id: BigInt(userId),
        type:    'plan_activated',
        title:   discountAmount > 0 ? 'Plan Activated + Discount Applied 🎉' : 'Plan Activated 🎉',
        body:    notifBody,
      })
      .execute();

    // 9. Record coupon use (invalidates it for re-use)
    if (couponId) {
      ...
      // If it was a referral coupon, mark the referral rewarded + notify referrer
      ...
      if (referralRow) {
        await trx
          ...
          .set({ status: 'rewarded', referrer_reward: String(discountAmount) })
        ...
        await trx
          ...
          .values({
            ...
            body:    'Someone you referred just activated their first Speedonet plan! Your referral has been marked as successful.',
          })
          .execute();
      }
    }
...
// ── Route handler helper — coupon pre-validation endpoint ─────────────────────
// POST /plans/coupon/validate  { plan_id, coupon_code }
...
async function validateCouponForPlan(userId, planId, couponCode) {
  ...
  return {
    ...
    discount_amount: result.discount,   // actual ₹ deducted
